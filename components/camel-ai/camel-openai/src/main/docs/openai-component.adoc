...
[NOTE]
====
The API key can be omitted if using OpenAI-compatible providers that don't require authentication (for example, some local LLM servers).
====
...
[NOTE]
====
When using image files, the userMessage is required. Supported image formats are detected by MIME type (for example, `image/png`, `image/jpeg`, `image/gif`, `image/webp`).
====
...
When `outputClass` is set, the model is instructed to produce JSON matching the given class, but the component returns the raw String. Deserialize the body yourself (for example, with Camel's Jackson) if you need a typed object.
...
This component works with any OpenAI API-compatible endpoint by setting the `baseUrl` parameter.

Supported endpoints:

- link:https://docs.redhat.com/en/documentation/red_hat_ai_inference_server/[RedHat Inference Server]
- link:https://api.openai.com/v1[OpenAI official API]
- link:https://ollama.com/[Ollama] Local LLM server

It is possible to use Third-party OpenAI-compatible providers, but these are not supported, and may require additional configuration.
...
| RedHat Inference Server | `sentence-transformers/all-MiniLM-L6-v2`
| OpenAI | `text-embedding-3-large`
| Ollama | `nomic-embed-text`

|===

.Generic example for local embeddings:
[source,yaml]
----
- to:
    uri: openai:embeddings
    parameters:
      # Replace parameters with your model
      baseUrl: http://your.model.domain/v1
...
.Ollama database integration example
[source,yaml]
----
